...
class PlatePosition < InventoryDB
  ...
  def self.find_all_positions(condition_array=nil)
    self.includes(:plate_tube).order('plate_positions.plate_or_tube_name, plate_position').where(sql_where(condition_array)).all
  ...
  def self.find_all_incl_oligos(condition_array=nil)
    self.includes(:synth_oligos).order('plate_positions.plate_or_tube_name, plate_position').where(sql_where(condition_array)).all
